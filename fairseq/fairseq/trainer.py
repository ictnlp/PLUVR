...
class Trainer(object):
    ...
    def get_transformer_params(self):
        for name, param in self.model.named_parameters():
            if 'vae' not in name and param.requires_grad:
                yield param
    
    def get_vae_params(self):
        for name, param in self.model.named_parameters():
            if 'vae' in name and param.requires_grad:
                yield param
    
    def get_criterion_params(self):
    ...
    def _build_optimizer(self):
        params = [
            {'params': self.get_transformer_params()},
            {'params': self.get_vae_params(), 'lr': self.cfg.optimization.vae_lr}
        ]
    ...
    def lr_step_update(self):
        ...
        new_lr, new_vae_lr = self.lr_scheduler.step_update(self.get_num_updates())
        metrics.log_scalar("lr", new_lr, weight=0, priority=300)
        metrics.log_scalar("vae_lr", new_vae_lr, weight=0, priority=300)
        return new_lr, new_vae_lr
